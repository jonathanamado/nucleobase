// app/publicacao_depoimentos/page.tsx
"use client";
...
export default function PublicacaoDepoimentos() {
  ...
  useEffect(() => {
    window.dataLayer?.push({
      ...
      content_category: "depoimentos",
      content_name: "pagina_publicacao_depoimentos"
    });
  ...
  const trackClick = (label: string, destination: string) => {
    window.dataLayer?.push({
      ...
      page_location: "/publicacao_depoimentos"
    });
  };
  ...
  const handleSubmitDepoimento = async () => {
    ...
    try {
      ...
      trackClick("Publicar Experiência Sucesso", "depoimentos_submitted");
      setEnviado(true);
    } catch (error: any) {
  ...
  if (enviado) {
    return (
      <div className="w-full max-w-2xl mx-auto flex flex-col items-center justify-center py-20 text-center animate-in fade-in zoom-in duration-500">
        ...
        <a href="/depoimentos" onClick={() => trackClick("Voltar para a Comunidade", "/depoimentos")} className="px-10 py-4 bg-gray-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-black transition-all shadow-xl cursor-pointer">
    ...
    <div className="w-full md:pr-10 animate-in fade-in slide-in-from-bottom-6 duration-700 pb-20 relative px-4 md:px-0">
              ...
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-700">
                  ...
                  <div className="flex gap-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button
                        ...
                        onClick={() => {
                          setRating(star);
                          trackClick(`Selecionar Nota: ${star} Estrelas`, "star_rating");
                        }}
                        onMouseEnter={() => setHover(star)}
                        onMouseLeave={() => setHover(0)}
                        className="transform transition-transform active:scale-90 cursor-pointer"
                      >
                        <Star
                          size={42}
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  <button
                    ...
                    disabled={loading}
                    className="w-full sm:flex-[2] bg-blue-600 text-white py-5 rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] hover:bg-blue-700 transition-all flex items-center justify-center gap-3 disabled:opacity-50 shadow-xl shadow-blue-100 cursor-pointer"
                  >
                    {loading ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} />}
                    {loading ? "Processando..." : "Publicar Experiência"}
                  </button>
                  <a href="/depoimentos" onClick={() => trackClick("Cancelar Publicação", "/depoimentos")} className="w-full sm:flex-1 px-8 py-5 border border-gray-100 text-gray-400 rounded-2xl hover:text-gray-900 hover:bg-gray-50 transition-all font-black text-[11px] uppercase tracking-[0.2em] flex items-center justify-center gap-3 cursor-pointer">
      ...
      <div className="flex flex-col items-center text-center">
        ...
        <a
          ...
          onClick={() => trackClick("Instagram - Publicação Depoimentos", "https://www.instagram.com/nucleobase.app/")}
